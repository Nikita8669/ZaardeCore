...
====================================
This guide will show you how to build zaarded (headless client) for OSX.
...
5.  (Optional) You can also install zaarded to your path:
...
4. Enter "zaarde-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `zaarded` for your own use.

zaarded/zaarde-cli binaries are not included in the zaarde-Qt.app bundle.

If you are building `zaarded` or `zaarde-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the Zaarde-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./zaarded`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./zaarded` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=zaarderpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/Zaarde/zaarde.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/Zaarde/zaarde.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/Zaarde/debug.log

Other commands:
-------

    ./zaarded -daemon # to start the zaarde daemon.
    ./zaarde-cli --help  # for a list of command-line options.
    ./zaarde-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure Zaarde with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the Zaarde wallet:
...
You have the choice to build the GUI Zaarde wallet as a Mac OSX app, described in “How to build the Zaarde-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the Zaarde-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find Zaarde-Qt.dmg inside your Zaarde folder. Open and install the wallet like any typical Mac app.
...
~/Downloads/Zaarde/src/qt/zaarde-qt