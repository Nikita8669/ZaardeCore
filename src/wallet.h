...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ZARD at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zZARD send
enum ZerocoinSpendStatus {
    ZZARD_SPEND_OKAY = 0,                            // No error
    ZZARD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZZARD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZZARD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZZARD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZZARD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZZARD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZZARD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZZARD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZZARD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZZARD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZZARD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZZARD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZZARD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZZARD_SPENT_USED_ZZARD = 14,                      // Coin has already been spend
    ZZARD_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZZARD_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool CreateZZARDOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzZARDWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzZARDTracker> zphrTracker;
    ...
    void setZWallet(CzZARDWallet* zwallet)
    {
        zwalletMain = zwallet;
        zphrTracker = std::unique_ptr<CzZARDTracker>(new CzZARDTracker(strWalletFile));
    }

    CzZARDWallet* getZWallet() { return zwalletMain; }
