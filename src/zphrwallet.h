...
#ifndef ZAARDE_ZZARDWALLET_H
#define ZAARDE_ZZARDWALLET_H
...
public:
    CzZARDWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZZARD(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZZARD(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ZAARDE_ZZARDWALLET_H
