...
CzZARDWallet::CzZARDWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZZARDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZZARDSeed_deprecated()) {
                    LogPrintf("%s: Updated zZARD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzZARDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZZARDCount(nCountLastUsed);
    else if (!walletdb.ReadZZARDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzZARDWallet::Lock()
{
    seedMaster = 0;
}

void CzZARDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzZARDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZZARD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzZARDWallet::LoadMintPoolFromDB()
{
...
void CzZARDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzZARDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzZARDWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzZARDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZZARD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzZARDWallet::SeedToZZARD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzZARDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzZARDWallet::GenerateDeterministicZZARD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzZARDWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzZARDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
