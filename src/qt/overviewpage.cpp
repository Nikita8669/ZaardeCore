...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ZARD)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sZARDPercentage, QString& szZARDPercentage)
{
    int nPrecision = 2;
    ...
    szZARDPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sZARDPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    // ui->labelZARDPercent->setText(sPercentage);
    // ui->labelzZARDPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zZARD.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in zaarde.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in zaarde.conf");
    }
    ...
    bool showZARDAvailable = settingShowAllBalances || phrAvailableBalance != nTotalBalance;
    bool showWatchOnlyZARDAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showZARDPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyZARDPending = watchUnconfBalance != 0;
    bool showZARDLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyZARDLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showZARDAvailable || showWatchOnlyZARDAvailable);
    ui->labelBalanceText->setVisible(showZARDAvailable || showWatchOnlyZARDAvailable);
    ui->labelWatchAvailable->setVisible(showZARDAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showZARDPending || showWatchOnlyZARDPending);
    ui->labelPendingText->setVisible(showZARDPending || showWatchOnlyZARDPending);
    ui->labelWatchPending->setVisible(showZARDPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showZARDLocked || showWatchOnlyZARDLocked);
    ui->labelLockedBalanceText->setVisible(showZARDLocked || showWatchOnlyZARDLocked);
    ui->labelWatchLocked->setVisible(showZARDLocked && showWatchOnly);
    ...
    bool showzZARDAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzZARDUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzZARDImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    // ui->labelzBalanceMature->setVisible(showzZARDAvailable);
    // ui->labelzBalanceMatureText->setVisible(showzZARDAvailable);
    // ui->labelzBalanceUnconfirmed->setVisible(showzZARDUnconfirmed);
    // ui->labelzBalanceUnconfirmedText->setVisible(showzZARDUnconfirmed);
    // ui->labelzBalanceImmature->setVisible(showzZARDImmature);
    // ui->labelzBalanceImmatureText->setVisible(showzZARDImmature);
    //bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    //ui->labelZARDPercent->setVisible(showPercentages);
    // ui->labelzZARDPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ZARD")
    updateDisplayUnit();
}
