...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ZARD);
    ...
    if (!settings.contains("nAnonymizeZaardeAmount"))
        settings.setValue("nAnonymizeZaardeAmount", 1000);

    nAnonymizeZaardeAmount = settings.value("nAnonymizeZaardeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeZaardeAmount"))
        SoftSetArg("-anonymizezaardeamount", settings.value("nAnonymizeZaardeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in zaarde.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZaardeAmount:
            return QVariant(nAnonymizeZaardeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZaardeAmount:
            nAnonymizeZaardeAmount = value.toInt();
            settings.setValue("nAnonymizeZaardeAmount", nAnonymizeZaardeAmount);
            emit anonymizeZaardeAmountChanged(nAnonymizeZaardeAmount);
            break;
        case CoinControlFeatures:
