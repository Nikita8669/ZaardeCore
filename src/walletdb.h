...
public:
    ...
    bool WriteZZARDSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZZARDSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZZARDSeed_deprecated(uint256& seed);
    bool EraseZZARDSeed();
    bool EraseZZARDSeed_deprecated();

    bool WriteZZARDCount(const uint32_t& nCount);
    bool ReadZZARDCount(uint32_t& nCount);
