...
// Copyright (c) 2017-2019 The Zaarde Developers
...
#if defined(NDEBUG)
#error "Zaarde cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock) {
    //Check to see if the zZARD is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zZARD spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zZARD without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zZARD spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zZARD spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zZARD spend tx %s already in block %d",
            ...
            // Check that zZARD mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
{
	int64_t nSubsidy = 0;
    if (nHeight == 0) {nSubsidy = 415000 * COIN;} //Premine
    else if (nHeight <= 100 && nHeight > 0) {nSubsidy = 0 * COIN;}
    else if (nHeight <= 10580 && nHeight > 100) {nSubsidy = 1 * COIN;} 
    else if (nHeight < 80000) {nSubsidy = 1.1 * COIN;}
    else if (nHeight < 120000) {nSubsidy = 1.2 * COIN;}
    else if (nHeight < 160000) {nSubsidy = 1.3 * COIN;}
    else if (nHeight < 200000) {nSubsidy = 1.4 * COIN;}
    else if (nHeight < 240000) {nSubsidy = 1.5 * COIN;}
    else if (nHeight < 280000) {nSubsidy = 1.6 * COIN;}
    else if (nHeight < 320000) {nSubsidy = 1.7 * COIN;}
	else if (nHeight < 360000) {nSubsidy = 1.8 * COIN;}
	else if (nHeight < 400000) {nSubsidy = 1.9 * COIN;}
	else if (nHeight < 440000) {nSubsidy = 2 * COIN;}
	else if (nHeight < 480000) {nSubsidy = 2.1 * COIN;}
	else if (nHeight < 520000) {nSubsidy = 3.3 * COIN;}
	else if (nHeight < 1000000) {nSubsidy = 3.3 * COIN;} 
	else if (nHeight < 1500000) {nSubsidy = 4.5 * COIN;} 
	else if (nHeight < 2000000) {nSubsidy = 3.9 * COIN;}
	else if (nHeight < 2500000) {nSubsidy = 3.3 * COIN;}
	else if (nHeight < 3000000) {nSubsidy = 2.7 * COIN;}
	else if (nHeight < 3500000) {nSubsidy = 2.1 * COIN;}
	else if (nHeight < 4000000) {nSubsidy = 1.5 * COIN;}
	else if (nHeight < 4500000) {nSubsidy = 1 * COIN;}
	else if (nHeight > 4500000) {nSubsidy = 1 * COIN;}
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60) && chainActive.Height() == 2000; // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Zaarde
...
{
    RenameThread("zaarde-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZZARDMinted()
{
...
void RecalculateZZARDSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zZARD supply
...
bool RecalculateZARDSupply(int nHeightStart)
{
...
bool UpdateZZARDSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    UpdateZZARDSupply(block, pindex);
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zZARDSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ZARD to zZARD
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zZARD spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zZARD serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zZARD mints and %d zZARD spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/Zaarde Core:1.1.0/" ||
            pfrom->cleanSubVer == "/Zaarde Core:1.3.0/" ||
            pfrom->cleanSubVer == "/Zaarde Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // Zaarde: We use certain sporks during IBD, so check to see if they are
